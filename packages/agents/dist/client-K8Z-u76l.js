...
var RPCClientTransport = class {
  ...
  async send(message, options) {
    ...
    try {
      ...
      if (!result) return;
      const extra = options?.relatedRequestId
        ? { requestInfo: { headers: {} } }
        : void 0;
  ...
  async handle(message) {
    ...
    if (Array.isArray(message)) {
      ...
      for (const msg of message) {
        ...
        if (response !== void 0)
          if (Array.isArray(response)) responses.push(...response);
...
var MCPClientConnection = class {
  ...
  async fetchTools() {
    ...
    do {
      toolsResult = await this.client
        .listTools({ cursor: toolsResult.nextCursor })
  ...
  async fetchResources() {
    ...
    do {
      resourcesResult = await this.client
        .listResources({ cursor: resourcesResult.nextCursor })
        .catch(
  ...
  async fetchPrompts() {
    ...
    do {
      promptsResult = await this.client
        .listPrompts({ cursor: promptsResult.nextCursor })
  ...
  async fetchResourceTemplates() {
    ...
    do {
      templatesResult = await this.client
        .listResourceTemplates({ cursor: templatesResult.nextCursor })
        .catch(
  ...
  getTransport(transportType) {
    switch (transportType) {
      ...
      case "rpc":
        return new RPCClientTransport(this.options.transport);
      default:
...
var MCPClientManager = class {
  ...
  async _restoreServer(serverId, retry) {
    ...
    if (
      (
        await tryN(maxAttempts, async () => this.connectToServer(serverId), {
  ...
  validateCallbackRequest(req) {
    ...
    if (!this.getServersFromStorage().some((server) => server.id === serverId))
      return {
        serverId,
        valid: false,
        ...
        entries.push([
          toolKey,
          {
            ...
            inputSchema: tool.inputSchema
              ? this.jsonSchema(tool.inputSchema)
              : this.jsonSchema({ type: "object" }),
            outputSchema: tool.outputSchema
              ? this.jsonSchema(tool.outputSchema)
              : void 0
          }
        ]);
      } catch (e) {
...
function getNamespacedData(mcpClients, type) {
  return Object.entries(mcpClients)
    .map(([name, conn]) => {
      return {
        data: conn[type],
        name
...
//#endregion
export {
  RPCServerTransport as a,
  RPCClientTransport as i,
  getNamespacedData as n,
  RPC_DO_PREFIX as o,
  MCPConnectionState as r,
  DisposableStore as s,
