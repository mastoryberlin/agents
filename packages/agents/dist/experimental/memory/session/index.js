...
function parseMicroCompactionRules(config) {
  ...
  const truncateToolOutputs =
    config.truncateToolOutputs === false
      ...
      : config.truncateToolOutputs === true ||
          config.truncateToolOutputs === void 0
  ...
  const truncateText =
    config.truncateText === false
      ? false
      : config.truncateText === true || config.truncateText === void 0
...
function estimateMessageTokens(messages) {
  ...
  for (const msg of messages) {
    ...
    for (const part of msg.parts)
      if (part.type === "text") tokens += estimateStringTokens(part.text);
...
var Session = class {
  constructor(storage, options) {
    ...
    this.microCompactionRules = parseMicroCompactionRules(
      options?.microCompaction ?? true
    );
  ...
  shouldAutoCompact() {
    ...
    return (
      estimateMessageTokens(this.storage.getMessages()) >
      this.compactionConfig.tokenThreshold
    );
  }
};
//#endregion
