...
var Workspace = class {
  ...
  resolveR2Prefix() {
    ...
    if (!name)
      throw new Error(
        "[Workspace] R2 is configured but no r2Prefix was provided and host.name is not available. Either pass r2Prefix in WorkspaceOptions or ensure the host exposes a name property."
      );
    return name;
  }
  r2Key(filePath) {
...
var WorkspaceFileSystem = class {
  ...
  async readFile(path, _options) {
    ...
    if (content === null)
      throw Object.assign(/* @__PURE__ */ new Error(`ENOENT: ${path}`), {
        code: "ENOENT"
      });
    return content;
  }
  ...
  resolvePath(base, path) {
    const parts = (path.startsWith("/") ? path : `${base}/${path}`)
      .split("/")
      .filter(Boolean);
    const resolved = [];
