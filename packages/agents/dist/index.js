...
function extractInternalFlags(raw) {
  const result = {};
  for (const key of Object.keys(raw))
    if (CF_INTERNAL_KEYS.has(key)) result[key] = raw[key];
  return result;
}
 ...
 */
const CONTROL_CHAR_RE = /* @__PURE__ */ new RegExp(
  "[\\u0000-\\u0008\\u000B\\u000C\\u000E-\\u001F\\u007F]",
  "g"
);
...
var Agent = class Agent extends Server {
  ...
  constructor(ctx, env) {
    ...
    );
    {
      ...
      if (hasOwnNew && hasOwnOld)
        throw new Error(
          "[Agent] Cannot override both onStateChanged and onStateUpdate. Remove onStateUpdate — it has been renamed to onStateChanged."
        );
      if (hasOwnOld) {
        const ctor = this.constructor;
        ...
        async () => {
          ...
          if (this.shouldSendProtocolMessages(connection, ctx)) {
            if (this._resolvedOptions.sendIdentityOnConnect) {
              const ctor = this.constructor;
              if (
                ctor.options?.sendIdentityOnConnect === void 0 &&
                ...
                if (!new URL(ctx.request.url).pathname.includes(this.name)) {
                  ...
                  console.warn(
                    `[Agent] ${ctor.name}: sending instance name "${this.name}" to clients via sendIdentityOnConnect (the name is not visible in the URL with custom routing). If this name is sensitive, add \`static options = { sendIdentityOnConnect: false }\` to opt out. Set it to true to silence this message.`
                  );
                }
              }
              connection.send(
    ...
    this.onStart = async (props) => {
      return __DO_NOT_USE_WILL_BREAK__agentContext.run(
        ...
        async () => {
          if (await this.ctx.storage.get("cf_agents_is_facet"))
            this._isFacet = true;
        ...
        const breakdown =
          active > 0 && completed > 0
            ...
            : active > 0
              ...
              : ` (${completed} completed)`;
        console.warn(
          `[Agent] Found ${total} workflow(s) referencing unknown binding '${oldName}'${breakdown}. If you renamed the binding, call: ${suggestion}`
        );
      }
    }
  }
  ...
  _broadcastProtocol(msg, excludeIds = []) {
    ...
    for (const conn of this.getConnections())
      if (!this.isConnectionProtocolEnabled(conn)) exclude.push(conn.id);
  ...
  _setStateInternal(nextState, source = "server") {
    ...
    const { connection, request, email } =
      __DO_NOT_USE_WILL_BREAK__agentContext.getStore() || {};
    this.ctx.waitUntil(
      (async () => {
        try {
      ...
      value(stateOrFn) {
        ...
        const flags =
          raw != null && typeof raw === "object"
            ? extractInternalFlags(raw)
            : {};
        const hasFlags = Object.keys(flags).length > 0;
        let newUserState;
        if (typeof stateOrFn === "function")
          newUserState = stateOrFn(hasFlags ? stripInternalKeys(raw) : raw);
  ...
  isConnectionReadonly(connection) {
    this._ensureConnectionWrapped(connection);
    return !!this._rawStateAccessors.get(connection).getRaw()?.[
      CF_READONLY_KEY
    ];
  }
  /**
    ...
    return __DO_NOT_USE_WILL_BREAK__agentContext.run(
      ...
      async () => {
        ...
        if ("onEmail" in this && typeof this.onEmail === "function")
          return this._tryCatch(() => this.onEmail(email));
        else {
  ...
  async replyToEmail(email, options) {
    return this._tryCatch(async () => {
      if (email._secureRouted && options.secret === void 0)
        throw new Error(
          "This email was routed via createSecureReplyEmailResolver. You must pass a secret to replyToEmail() to sign replies, or pass explicit null to opt-out (not recommended)."
        );
  ...
  async schedule(when, callback, payload, options) {
    if (this._isFacet)
      throw new Error(
        "Scheduling is not supported in sub-agents. Schedule from the parent agent instead."
      );
    const id = nanoid(9);
    if (options?.retry)
      validateRetryOptions(options.retry, this._resolvedOptions.retry);
  ...
  getSchedules(criteria = {}) {
    ...
    if (criteria.timeRange) {
      ...
      const end =
        criteria.timeRange.end || /* @__PURE__ */ new Date(999999999999999);
      params.push(
  ...
  async keepAlive() {
    if (this._isFacet)
      throw new Error(
        "keepAlive() is not supported in sub-agents. Use keepAlive() from the parent agent instead."
      );
    ...
    if (result && Array.isArray(result))
      for (const row of result) {
        ...
        if (row.type === "cron") {
          ...
          const nextTimestamp =
            Math.floor(Date.now() / 1e3) + (row.intervalSeconds ?? 0);
          this.sql`
  ...
  async subAgent(cls, name) {
    ...
    if (!ctx.facets || !ctx.exports)
      throw new Error(
        'subAgent() requires the "experimental" compatibility flag. Add it to your wrangler.jsonc compatibility_flags.'
      );
    if (!ctx.exports[cls.name])
      throw new Error(
        `Sub-agent class "${cls.name}" not found in worker exports. Make sure the class is exported from your worker entry point and that the export name matches the class name.`
      );
    const facetKey = `${cls.name}\0${name}`;
    const stub = ctx.facets.get(facetKey, () => ({
      class: ctx.exports[cls.name]
    }));
  ...
  async terminateWorkflow(workflowId) {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes("Not implemented"))
        throw new Error(
          "terminateWorkflow() is not supported in local development. Deploy to Cloudflare to use this feature. Follow https://github.com/cloudflare/agents/issues/823 for details and updates."
        );
      throw err;
    }
  ...
  async pauseWorkflow(workflowId) {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes("Not implemented"))
        throw new Error(
          "pauseWorkflow() is not supported in local development. Deploy to Cloudflare to use this feature. Follow https://github.com/cloudflare/agents/issues/823 for details and updates."
        );
      throw err;
    }
    const status = await instance.status();
  ...
  async resumeWorkflow(workflowId) {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes("Not implemented"))
        throw new Error(
          "resumeWorkflow() is not supported in local development. Deploy to Cloudflare to use this feature. Follow https://github.com/cloudflare/agents/issues/823 for details and updates."
        );
      throw err;
    }
    const status = await instance.status();
  ...
  async restartWorkflow(workflowId, options = {}) {
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes("Not implemented"))
        throw new Error(
          "restartWorkflow() is not supported in local development. Deploy to Cloudflare to use this feature. Follow https://github.com/cloudflare/agents/issues/823 for details and updates."
        );
      throw err;
    }
    if (resetTracking) {
  ...
  getWorkflows(criteria = {}) {
    ...
    const workflows = (hasMore ? rows.slice(0, limit) : rows).map((row) =>
      this._rowToWorkflowInfo(row)
    );
    return {
      workflows,
      total,
  ...
  _updateWorkflowTracking(workflowId, status) {
    ...
    const now = Math.floor(Date.now() / 1e3);
    const completedAt = ["complete", "errored", "terminated"].includes(
      statusName
    )
      ? now
      : null;
  ...
  _rowToWorkflowInfo(row) {
    return {
      ...
      completedAt: row.completed_at
        ? /* @__PURE__ */ new Date(row.completed_at * 1e3)
        : null
    };
  }
  /**
  ...
  async addMcpServer(
    ...
    if (
      ...
      throw new Error(
        "callbackPath is required in addMcpServer options when sendIdentityOnConnect is false — the default callback URL would expose the instance name. Provide a callbackPath and route the callback request to this agent via getAgentByName."
      );
