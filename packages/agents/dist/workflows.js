...
var AgentWorkflow = class extends WorkflowEntrypoint {
  ...
  async _initAgent(agentName, agentBinding, workflowName, instanceId) {
    if (!agentName || !agentBinding || !workflowName)
      throw new Error(
        "AgentWorkflow requires __agentName, __agentBinding, and __workflowName in params. Use agent.runWorkflow() to start workflows with proper agent context."
      );
  ...
  async waitForApproval(step, options) {
    ...
    const payload = (
      await step.waitForEvent(stepName, {
        type: eventType,
        timeout
      })
    ).payload;
