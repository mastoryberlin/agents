...
import {
  a as RPCServerTransport,
  i as RPCClientTransport,
  o as RPC_DO_PREFIX
...
var StreamableHTTPServerTransport = class {
    ...
    if (!hasRequests)
      for (const message of messages) {
        if (this.messageInterceptor) {
          if (
            await this.messageInterceptor(message, {
              authInfo,
              requestInfo
            })
          )
  ...
  async send(message, options) {
    ...
    const connection = Array.from(agent.getConnections()).find((conn) =>
      conn.state?.requestIds?.includes(requestId)
    );
    if (!connection)
      throw new Error(
...
var WorkerTransport = class {
  ...
  async handleGetRequest(request) {
    ...
    if (this.streamMapping.get(streamId) !== void 0)
      return new Response(
        JSON.stringify({
          jsonrpc: "2.0",
          error: {
      ...
      if (relatedIds.every((id) => this.requestResponseMap.has(id))) {
        if (this.enableJsonResponse && response.resolveJson) {
          const responses = relatedIds.map((id) =>
            this.requestResponseMap.get(id)
          );
          const headers = new Headers({
            "Content-Type": "application/json",
            ...this.getHeaders()
  ...
  async onSSEMcpMessage(_sessionId, messageBody, extraInfo) {
    if (this.getTransportType() !== "sse")
      return /* @__PURE__ */ new Error(
        "Internal Server Error: Expected SSE transport"
      );
    try {
      let parsedMessage;
      try {
  ...
  static serve(
    ...
    return {
      async fetch(request, env, ctx) {
        ...
        switch (transport) {
          case "streamable-http":
            return createStreamingHttpHandler(path, namespace, {
              corsOptions,
              jurisdiction
            })(request, ctx);
          case "sse":
            return createLegacySseHandler(path, namespace, {
              corsOptions,
              jurisdiction
            })(request, ctx);
          default:
